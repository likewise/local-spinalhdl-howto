# Checks out SpinalHDL v1.6.4 (master)
# Checks out VexRiscv (master)

# Builds SpinalHDL locally
# and tries to build VexRiscv against locally built SpinalHDL

# If you change 1.6.4 below, change all occurences! (@TODO variable)
...
try/VexRiscv:
	...
	sed -i "s@val spinalVersion.*=.*@val spinalVersion = \"1.6.4\"@" VexRiscv/build.sbt
	cat VexRiscv/build.sbt
	cd ..
